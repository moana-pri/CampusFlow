  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Registration flow
        const { data: authData, error: signUpError } = await auth.signUp(email, password, { name, role });
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-4 rounded-2xl bg-gradient-to-r ${config.gradient} text-white hover:shadow-lg transition-all shadow-${config.color}-600/25 disabled:opacity-50`}
            >
            ...
            {/* Google Sign In - Temporarily hidden until OAuth configured */}
            {false && (
              <button
                type="button"
                onClick={async () => {
                ...
                disabled={loading}
                className="w-full py-4 rounded-2xl border-2 border-slate-200 bg-white hover:bg-slate-50 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
              >
